...
+ (UserInfos *)sharedUserInfo {
    ...
    @synchronized(self) {
        if (!userInfo) {
            ...
            NSString *filePath = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/user.arc"];
            if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
                UserInfos *localUser = [NSKeyedUnarchiver unarchiveObjectWithFile:filePath];
