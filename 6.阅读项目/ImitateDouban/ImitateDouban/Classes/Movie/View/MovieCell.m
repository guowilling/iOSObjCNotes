...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    ...
    static NSString *cellIndertifer = @"movieCell";
    MovieCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndertifer];
    if (!cell) {
        cell = [[MovieCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIndertifer];
    }
    return cell;
}
...
- (void)setupContentLayout {
    ...
    int padding1 = 5;
    int padding2 = 10;
    CGFloat width = SCREEN_WIDTH - 3 * magin - 80;
    ...
    [_titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_iconImageView).offset(padding1);
    ...
    [_starView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_titleLabel.mas_bottom).offset(padding2);
    ...
    [_averageLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(_starView.mas_right).offset(padding2);
    }];
    
    [_directorLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_starView.mas_bottom).offset(padding2);
        make.left.with.height.with.equalTo(_titleLabel);
    }];
    
    [_castsLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_directorLabel.mas_bottom).offset(padding1);
        make.width.mas_equalTo(@(width));
        make.left.equalTo(_titleLabel.mas_left);
    }];
...
+ (CGFloat)getCellHeight:(MovieModel *)model {
    ...
    NSString *casString = [NSString stringWithFormat:@"演员: %@",[casArr componentsJoinedByString:@"/ "]];
    CGSize casSize = [casString attrStrSizeWithFont:[UIFont systemFontOfSize:IntroduceFont] maxSize:maxSize lineSpacing:4];
