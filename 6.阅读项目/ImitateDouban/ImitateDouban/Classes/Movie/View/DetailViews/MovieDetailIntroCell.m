...
- (void)configCellWithModel:(DetailMovieModel *)model {
    ...
    if ([NSString isAvailable:model.summary]) {
        introduceStr = @"";
    }else{
...
+ (CGFloat)heightWithModel:(DetailMovieModel *)model {
    ...
    CGSize titleSize = [model.summary attrStrSizeWithFont:[UIFont systemFontOfSize:IntroduceFont] maxSize:maxSize lineSpacing:6];
    if (model.isExpanded) {
