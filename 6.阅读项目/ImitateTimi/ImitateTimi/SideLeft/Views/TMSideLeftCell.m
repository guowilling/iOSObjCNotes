...
- (void)longGR:(UILongPressGestureRecognizer *)longPress {
    ...
    if ([self.sideCellDelegate respondsToSelector:@selector(sideLeftCellWithIndexPath:withLongPress:)]) {
        [self.sideCellDelegate sideLeftCellWithIndexPath:self.indexPath withLongPress:longPress];
    }
}
...
- (void)setBook:(TMBooks *)book {
    ...
    self.countLabel.text =  [NSString stringWithFormat:@"%lu笔", [[TMDataBaseManager defaultManager] numberOfAllBillCountWithBookID:book.booksID]];
}
